...
public class ItemLaunchWand extends Item {
    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        tooltip.add(new TranslatableText("item.whizzards.launch_wand.tooltip_1"));
    }
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        if (user instanceof PlayerEntity caster) {
            ...
            world.addParticle(ParticleTypes.FLASH, caster.getX(), caster.getY(), caster.getZ(), 1.0, 0.0, 0.0);
